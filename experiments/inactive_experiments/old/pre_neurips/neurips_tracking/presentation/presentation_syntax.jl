### Preliminaires ###
# Definitions of some probability distributions
moving_away_from_home(vₜ₋₁, xₜ₋₁) = sign(vₜ₋₁) == sign(xₜ₋₁ - HOME)
prior_p_stop_tired(eₜ₋₁) = -exp(eₜ₋₁/10)
prior_p_stop_far(xₜ₋₁, vₜ₋₁) = moving_away_from_home(vₜ₋₁, xₜ₋₁) ? 1-exp(-abs(xₜ₋₁-HOME)/10) : 0.
prop_p_stop_far(is_stopped, vₜ₋₁, xₜ₋₁) = !is_stopped ? 0. : moving_away_from_home(vₜ₋₁, xₜ₋₁) ? 0.5 : 0.
...
### Initial timestep model and proposal ###
...
@gen (static) function obs_model(eₜ, vₜ, xₜ)
    obsₜ ~ categorical(discretized_gaussian(xₜₜ, 2.0, Xs))
end
### Step Proposal ###
...
@gen (static) function vel_prop(vₜ, eₜ₋₁, vₜ₋₁, xₜ₋₁)
...
### Construct PseudoMarginal Estimator ###
vel_step_dist = PseudoMarginalize(vel_step, vel_prop, N_aux_particles=2)

### Build SMC circuit ###
hmm = HiddenMarkovModel(initial_model, step_model, obs_model)
smc_circuit = NeuralSMCCircuit(hmm, initial_proposal step_proposal, n_particles=10)