...
@gen (static) function step_model(dxₜ₋₁, dyₜ₋₁, dzₜ₋₁, xₜ₋₁, yₜ₋₁, zₜ₋₁, rₜ₋₁, true_ϕₜ₋₁, true_θₜ₋₁, dϕₜ₋₁, dθₜ₋₁)
    ...
    dϕ = { :dϕ } ~ LCat(SphericalVels())(truncated_discretized_gaussian(true_ϕ -  true_ϕₜ₋₁, .2, SphericalVels()))
    dθ = { :dθ } ~ LCat(SphericalVels())(truncated_discretized_gaussian(true_θ -  true_θₜ₋₁, .2, SphericalVels()))
    return (dxₜ, dyₜ, dzₜ, xₜ, yₜ, zₜ, rₜ, true_ϕ, true_θ, dϕ, dθ)
end
...
@gen (static) function step_proposal(dxₜ₋₁, dyₜ₋₁, dzₜ₋₁, xₜ₋₁, yₜ₋₁, zₜ₋₁,
    ...
    dϕ = { :dϕ } ~ LCat(θs())(truncated_discretized_gaussian(obs_ϕ - (true_ϕₜ₋₁ - dϕₜ₋₁), 0.2, ϕs()))    
    dθ = { :dθ } ~ LCat(θs())(truncated_discretized_gaussian(obs_θ - (true_θₜ₋₁ - dθₜ₋₁), 0.2, θs()))
...
#    r_probvec = normalize(vcat(ones(Int64(r_max)), zeros(Int64(l-r_max))))
 #   rₜ = { :rₜ } ~ LCat(Rs())(r_probvec)
    ...
    vxₜ = { :dx } ~ LCat(Vels())(unif(Vels()))
    vyₜ = { :dy } ~ LCat(Vels())(unif(Vels()))
    vzₜ = { :dz } ~ LCat(Vels())(unif(Vels()))
end
