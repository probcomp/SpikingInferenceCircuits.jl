...
@gen (static) function obs_proposal(occₜ, xₜ, yₜ, vxₜ, vyₜ, img)
    determ_img = img_determ_with_colors(occₜ, xₜ, yₜ)
    img_inner ~ Map(Map(fill_in_flips))(is_flipped_vec_of_vecs(determ_img, img))
end

initial_near_locopt_proposal = @compile_initial_proposal(_init_near_locopt_proposal, obs_proposal, 5, 1)
step_near_locopt_proposal = @compile_step_proposal(_step_near_locopt_proposal, obs_proposal, 5, 1)